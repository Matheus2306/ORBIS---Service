...
| F2 | P0 | EF, migrations, tenant filters, RLS e FK composta | Banco real, role restrita, ataques cross-tenant e pool reuse | pendente |
| F3 | P0 | API autenticada, memberships e resolução por host | Assinatura/issuer/audience + membership + recurso + contratos HTTP | pendente |
| F4 | P0 | Criação/aceite/conclusão e auditoria | Idempotência, 100 atualizações concorrentes, atomicidade | pendente |
