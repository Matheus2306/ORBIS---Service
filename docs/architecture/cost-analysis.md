...
Fórmula mensal: N_API×preço_instância + DB_primary+standby+IOPS+WAL + storage_GB×tarifa + egress_GB×tarifa + ingestão/retenção_observabilidade + backups_GB×tarifa + LB/DNS/edge + IdP_MAU + workers. Cache/queues externos=0 enquanto não adotados. N_API depende do benchmark e da reserva de falha, não de palpite. DB segue carga/IOPS/pool medidos; HA duplica parte do custo base mesmo com poucos usuários.
