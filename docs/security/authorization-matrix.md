...
`S`: somente próprio usuário/recurso; `A`: recurso atribuído; `T`: tenant atual conforme permissão explícita; `P`: control plane separado; `—`: negado. As colunas são **perfis candidatos**, não roles implementadas nem grants automáticos. Hoje existem somente sete flags de ordens; as demais permissões abaixo são propostas.
