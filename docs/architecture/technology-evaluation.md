...
| Kubernetes/service mesh | Dois hosts da mesma solução, sem necessidade operacional demonstrada | Não adotar; plataforma gerida é candidata; separação de credencial não justifica cluster |
| Orbis.Hosting + host administrativo ASP.NET Core | Separar credencial privilegiada sem duplicar controles HTTP | ADR-017; middleware nativo, sem plataforma nova; testes de audiência/MFA/grants; operação/benchmark pendentes |
