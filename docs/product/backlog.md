...
| F5 | P0 | Dataset Small e baseline Release | RPS/percentis/erros + métricas do gerador e servidor | em curso: Small, 960 amostras SQL, HTTPS/métricas e gerador fixado; 126 testes; host separado e baseline pendentes |
| F6 | P0 | CI reproduzível | Restore locked, format, build, testes, scanners e perf smoke | em curso: 12 checks locais e 26 regras aprovados; CI remoto/imagem/perf smoke pendentes |
...
Priorizar a [matriz de capabilities](../api/domain-capability-matrix.md) e o [catálogo](../api/endpoint-catalog.md), fonte das contagens atuais. Controllers, contexto de identidade/tenant/permissões e leitura de membros implementados; administração/provisionamento são os próximos incrementos P0. A base ainda não representa API BASELINE READY. F5/F6 continuam gates, sem alegação de performance preservada somente pelo build.

ADRs016/017 entregam núcleo e três controllers de alteração de acesso em host separado, com delegação/versão/último administrador/audit/idempotência, MFA/audiência/guard. Próximo incremento: convites vinculados a identidade verificada; sem escrita administrativa na API comum. IdP/primeiro administrador operacionais e F5/F6 permanecem pendentes.
