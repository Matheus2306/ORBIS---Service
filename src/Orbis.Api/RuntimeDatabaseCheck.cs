        ...
        await using var command = new NpgsqlCommand("""
            SELECT NOT (r.rolsuper OR r.rolbypassrls OR r.rolcreatedb OR r.rolcreaterole)
                ...
                AND (SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
                    WHERE n.nspname='public' AND c.relname IN ('memberships','work_orders','work_order_audit','order_creation_receipts','order_transition_receipts')
                ...
                AND NOT has_table_privilege(current_user, 'order_transition_receipts', 'UPDATE,DELETE,TRUNCATE')
