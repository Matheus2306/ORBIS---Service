...
A API expõe detalhe autenticado `GET /v1/work-orders/{id}`, criação idempotente `POST /v1/work-orders` com audit transacional, liveness, readiness PostgreSQL e OpenAPI em Development. Host verificado, identidade global, membership e permissão de recurso são obrigatórios. Consulte o [contrato/configuração](docs/api/work-orders.md), [evidência HTTP](docs/testing/reports/2026-09-17-authenticated-api.md) e [evidência de criação](docs/testing/reports/2026-09-17-idempotent-creation.md). Nenhum serviço foi publicado; fornecedor OIDC operacional e portais ainda estão pendentes.
