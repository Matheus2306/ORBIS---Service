...
Também estão implementados o [contexto atual](docs/api/current-context.md) e a [consulta de membros](docs/api/membership-read.md), com permissão ReadMembers, paginação e filtros. O [catálogo](docs/api/endpoint-catalog.md) distingue as 19 operações existentes (16 na API comum +3 administrativas) das capacidades ainda planejadas.

O [host administrativo de memberships](docs/api/membership-administration.md) expõe permissões, suspensão e reativação em processo/credencial/audiência separados, com MFA recente e delegação persistida. [Configuração para executar](docs/operations/administration-host.md). Preserva versão, último administrador e audit/recibo atômicos; IdP operacional pendente. [Validação do incremento](docs/testing/reports/2026-09-24-administrative-http.md), sem comprovação de capacidade ou produção.
