...
Control plane mínimo: Tenant(id,status,slug), TenantDomain(host,tenant,verified), User(id), ExternalIdentity(issuer,subject,user). Directory lookup não permite listar dados dos tenants. Host é único após normalização/prova de posse; tabelas globais não são catálogo público.
